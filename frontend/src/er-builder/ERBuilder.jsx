...
function prepareModelForMapping(currentModel) {
  return {
    ...
    relationships: currentModel.relationships.map(
      (relationship) => {
...
async function parseResponse(response) {
  ...
  if (!text) {
    throw new Error(
      `Mapping engine returned an empty response (HTTP ${response.status}).`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Mapping engine returned invalid JSON (HTTP ${response.status}).`,
    );
  }
}
...
export default function ERBuilder({
  ...
  const initialSavedState =
    useRef(loadSavedState()).current;

  const [model, setModel] = useState(
    initialSavedState.model,
  );
  ...
  useEffect(() => {
    const nextNodes = model.entities.map(
      (entity, index) => {
        const savedPosition =
  ...
  function handleAddEntity() {
    setModel((currentModel) =>
      addEntity(
        currentModel,
        `Entity${
          currentModel.entities.length + 1
        }`,
      ),
    );
  }
  ...
  async function handleGenerateSchema() {
    console.log(
      "Generate Relational Schema clicked",
    );
    ...
    try {
      const modelForMapping =
        prepareModelForMapping(model);

      console.log(
        "Sending model to mapping engine:",
        modelForMapping,
      );
      ...
      console.log(
        "Mapping engine response status:",
        response.status,
      );
      ...
      if (!response.ok) {
        ...
        throw new Error(
          backendErrors ||
            `Mapping engine returned HTTP ${response.status}.`,
        );
      }
      ...
      setMappingError(
        error.message ||
          "Could not connect to the mapping engine.",
      );
    } finally {
  ...
  function handleClear() {
    ...
    try {
      ...
      console.warn(
        "Could not clear saved diagram from localStorage.",
        error,
      );
    }
  }
        ...
        <section className="mapping-result-panel">
          ...
          {mappingResult
            ...
            <div className="schema-tables">
              {mappingResult.relational_schema.tables.map(
                (table) => (
                  <div
